...
class Fabricator(db.Model):
    ...
    def begin(self):
        try:
            ...
            # Auto-create issue for print start failures with deduplication
            ...
            if job:
                ...
                print(f"[Fabricator] Job {job.id} failed to start on {self.name}. Issue #{issue.get('issue_id') if issue else 'N/A'} {'(duplicate)' if issue.get('duplicate') else '(new)'}")
    ...
    def handleVerdict(self):
        # Thread-safe verdict handling to prevent race conditions
        with self._status_lock:
            ...
            if self.device.verdict == "complete":
                ...
                from Classes.Jobs import Job as JobClass
                JobClass.update_job_status(job.id, "error")
                # Auto-create issue for misprint with deduplication
                from Classes.Issues import Issue
                issue = Issue.create_issue_from_error(
                ...
                print(f"[Fabricator] Job {job.id} marked as misprint on {self.name}. Issue #{issue.get('issue_id') if issue else 'N/A'} {'(duplicate)' if issue.get('duplicate') else '(new)'}")
