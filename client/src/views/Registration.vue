...
<template>
  <transition name="slide-down" appear>
    <div class="flex flex-wrap justify-center items-start">
      <div class="md:w-1/3 mx-auto max-w-md space-y-4 my-10">
        <!-- Select Fabricator -->
        <label
          for="fabricatorSelect"
          class="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-100"
          >Select a Fabricator</label
        >
        <div class="flex items-center">
          ...
          <div class="ml-2">
            ...
            <button
              type="submit"
              class="h-full flex items-center justify-center bg-accent-primary hover:bg-accent-primary-dark text-white rounded-md transition duration-300"
              @click="refreshFabricatorList"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 80 80"
                class="w-7 h-7 my-1 mx-1 py-1"
              >
                <path
        ...
        <!-- Fabricator Name -->
        <div class="space-y-1">
          <label for="name" class="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-100"
            >Fabricator Name</label
          >
          <input
            id="name"
            type="text"
            ...
            v-model="customName"
            placeholder="Name your fabricator"
          />
        </div>

        <!-- Model (Disabled) -->
        <div class="space-y-1">
          <label for="model" class="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-100"
            >Model</label
          >
          <input
            id="model"
            type="text"
            class="input-style w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm bg-gray-100 cursor-not-allowed"
            disabled
        ...
        <button
          ...
          :disabled="!selectedFabricator"
          @click="handleSubmit"
        >
          Submit
        </button>
      </div>
      <div v-if="fabricatorList.length > 0" class="md:w-2/3 flex flex-wrap justify-center mt-10">
        <FabricatorCard
