...
import { api, doHandleApi } from './ports'
...
export async function jobTime(job: Job, printers: any) {
    if (printers) {
        ...
        const updateJobTime = () => {
            ...
            job.job_client!.total_time = job.job_server![0] * 1000
...
async function refetchTime(printerid: number, jobid: number) {
    return doHandleApi('refetchtimedata', {
        printerid,
        jobid
    }, 'refetch time data', 'updating the job status', true)
...
export function useGetFile() {
    return {
        async getFile(job: Job): Promise<File | undefined> {
            try {
                const response = await api(`getfile?jobid=${job.id}`)
